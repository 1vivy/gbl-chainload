...
BootFlowChainLoad (VOID)
{
  ...
  /* Proper transition: release the logfs partition handle so the next EFI in
     ...
     observed on infiniti as the patched ABL → recovery transition failing
     with a hard power-off (2026-05-13).

     A previous attempt deferred the close to LogFsLib's EBS callback in the
     hope that the patched ABL's verbose-tier hook traces (GBL_DBG_LOGFS_ONLY)
     would land in gbl-chainload_BootN.txt. They didn't, on two counts:
       (a) the partition-handle conflict bricked the recovery boot, and
       (b) writes between LoadImage and the patched ABL's EBS are only
           durable if the 4 KiB threshold-flush fires or EBS itself runs.
           When the patched ABL crashes pre-EBS (as it did here), every
           sub-threshold write since the last flush is lost.
     Net: keep this close. The verbose-tier infrastructure remains but the
     post-handoff portion is effectively unreachable. */
