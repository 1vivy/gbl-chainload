...
- **Mode-2 profile lifecycle is TOML-based.** Host tooling uses `gbl-chainload_profile.toml`; the release profile ZIP should consume or populate `/sdcard/gbl-chainload_profile.toml` from `/sdcard/stock_vbmeta.img` and fail closed on stale/missing data.
