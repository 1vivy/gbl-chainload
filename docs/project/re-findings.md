...
- `patch7` orange-state CBZ. The guard instruction is byte-identical across builds: `0x3400046A` = `CBZ W10, +0x8C` (imm19=35), rewritten to `0x14000023` (`B` same displacement). What moves between OTA builds is its surrounding context, so an EU-derived fixed byte anchor missed 201 (an extra `STR` shifts the CBZ from CSEL+4 to CSEL+8). `ApplyOrangeScreen` is two-tier, most-reliable first:
  - **Primary (string anchor):** scan for the warning text `"Your device has been unlocked and can't be trusted"` (invariant across OTA builds, referenced by exactly one ADRP+ADD), resolve that ADRP+ADD with `Arm64FindAdrpAddTargeting`, then take the nearest preceding `CBZ Wn` (within 0x40) as the guard and `RewriteBUncond` it (target preserved). Across EU-703 / IN-201 / fairlady-200 the CBZ is consistently ADRP-0x1C. The "Orange State" string itself has 2 ADRP refs (ambiguous) — use the longer "has been unlocked…" string, which has exactly 1.
  - **Fallback (instruction pattern):** the canoe `delay_anchor` — the CBZ (wildcarded, so a re-scan matches post-rewrite) + the masked 5-second-delay setup `__ 06 80 52 __ 00 00 __ __ 05 __ __`; site guard gives idempotency (already-`B`→OK) and covers builds lacking the string.
  - Each tier is proven to locate the CBZ independently (corrupt one anchor, the other still applies — `tests/patches/test_patch7.c` §7). Cross-build gate `tests/host/088_patch7_multi_abl.sh`: infiniti-EU-16.0.5.703 (CBZ @0x78F0), infiniti-IN-16.0.7.201 (@0x76D8), op15t-fairlady-201 (@0x76D8), fairlady-CN-16.0.7.200 (@0x76D8) all → OK + idempotent; non-oplus (xi17-pudding Xiaomi, myron) correctly get zero hits. Ghidra: bookmark+comment on the 201 CBZ @0x76D8 in the `gbl_root_canoe` project (`abl-201.efi`).
