...
`InstallSpssHook` returns `EFI_NOT_FOUND` with a benign `GBL_INFO` for honest absence, and surfaces the contract-violating `EFI_SUCCESS`+NULL-interface case as `EFI_DEVICE_ERROR` (distinct from absence). For mode-0/mode-1 (observation-only) the chain-load continues with `spss=0/1` — this is exactly what the macan mode-0 bundle shows.

**mode-2 on macan is deliberately NOT enabled by this work, and that is the safe choice.** `InstallAll` keeps SPSS install **fatal under `WantProfileSpoof`**, including `EFI_NOT_FOUND`. The reason is that `EFI_NOT_FOUND` is **ambiguous**: it is the same return whether the SoC genuinely has no SPU (macan) *or* an SPU-backed target (canoe/infiniti) whose SPSS DXE failed to publish `gEfiSPSSProtocolGuid` this boot. Treating all `NOT_FOUND` as benign would silently boot mode-2 on an SPU platform with the SPU KeyMint mirror unhooked — an incomplete spoof. So we fail closed (abort → FastbootLib). Enabling mode-2 on a *known* no-SPU SoC requires a **positive capability signal** (a future manifest flag the installer sets from `ro.board.platform`), not a blanket `NOT_FOUND` pass. Tracked as follow-up. (Caught in review — an earlier draft of this change made `NOT_FOUND` universally benign and introduced exactly this fail-open regression.)
