...
- `patch7` orange-state CBZ. The guard instruction is byte-identical across builds: `0x3400046A` = `CBZ W10, +0x8C` (imm19=35), rewritten to `0x14000023` (`B` same displacement). What moves between OTA builds is its surrounding context, so an EU-derived fixed anchor misses other builds. Confirmed offsets: EU-16.0.5.703 PE → CBZ at `0x78F0`; IN-16.0.7.201 PE → CBZ at `0x76D8`. In 201 an extra `STR` (`ff4307b9`) sits between the upstream `CSEL` (`36 31 88 1A`) and the CBZ, so the CBZ is at CSEL+8 (vs CSEL+4 on EU) — that shift is why the canoe primary CSEL+4 anchor and our EU 24-byte anchor both missed 201. Durable anchor (matches both builds, unique in each): exact CBZ `6A 04 00 34` followed by the masked 5-second-delay setup (canoe `delay_anchor`) `__ 06 80 52 __ 00 00 __ __ 05 __ __` (mask `FF FF FF FF 00 FF FF FF 00 FF FF 00 00 FF 00 00`); rewrite at anchor offset 0. Reproduce: `fv-unwrap <abl.img> pe.efi` then scan the PE.
