...
So `SpssHook: LocateProtocol → Not Found` on the diag unit is **not** "no SPU" — it is the SPU failing to come up *on that unit* (`SPSSLib_LoadSPSS … PMIC clients failed: 0xE`), so the SPSS DXE never published the protocol, so the ABL skipped the mirror (BSP `ShareKeyMintInfoWithSPU` NOT_FOUND branch). A *healthy* macan unit publishes SPSS, exactly like infiniti.
...
**fail-closed is therefore correct, and there is no no-SPU flag to build.** `InstallAll` keeps SPSS install **fatal under `WantProfileSpoof`** (incl. `NOT_FOUND`): on a macan unit whose SPU is up, SPSS publishes and the hook installs (full spoof, like infiniti); on a unit whose SPU is down, refusing mode-2 is right — shipping a KM/QSEECOM-only spoof while a StrongBox SPU domain exists is the incomplete-spoof Codex flagged. `SpssHook` returns benign `NOT_FOUND` (observation-only modes continue `spss=0/1`, as the bundle shows) and `EFI_DEVICE_ERROR` for the `EFI_SUCCESS`+NULL contract violation.

**Open question for device test (decides nothing in code, only validation):** is the diag unit's SPU-init failure (PMIC `0xE`) representative of macan, or a degraded/flaky unit? On a healthy unit mode-2 should work unchanged. Have the owner run mode-2 staged on a healthy macan and report `spss=1/1` (SPU up → mode-2 works) vs `spss=0/1`+abort (SPU down). Only if SPU-down proves *universal* on macan would an operator-opt-in KM-only-spoof capability become worth designing — and that needs the device evidence first.
