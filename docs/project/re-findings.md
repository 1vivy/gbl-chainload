...
- `patch7` orange-state CBZ. The guard instruction is byte-identical across builds: `0x3400046A` = `CBZ W10, +0x8C` (imm19=35), rewritten to `0x14000023` (`B` same displacement). What moves between OTA builds is its surrounding context, so an EU-derived fixed anchor misses other builds. In 201 an extra `STR` (`ff4307b9`) sits between the upstream `CSEL` (`36 31 88 1A`) and the CBZ, so the CBZ is at CSEL+8 (vs CSEL+4 on EU) — that shift is why the canoe primary CSEL+4 anchor and our EU 24-byte anchor both missed 201. Durable anchor (matches every oplus build, unique in each): the CBZ word (WILDCARDED so a re-scan still matches after rewrite) followed by the masked 5-second-delay setup (canoe `delay_anchor`) `__ 06 80 52 __ 00 00 __ __ 05 __ __`; rewrite at anchor offset 0; `ApplyOrangeScreen` guards the site (already-`B` → idempotent OK; CBZ → rewrite; else → MISS). Cross-build validation (`tests/host/088_patch7_multi_abl.sh`, all → OK + idempotent): infiniti-EU-16.0.5.703 (CBZ @0x78F0), infiniti-IN-16.0.7.201 (@0x76D8), op15t-fairlady-201 (@0x76D8), fairlady-CN-16.0.7.200 (@0x76D8). Non-oplus ABLs correctly get zero hits: xi17-pudding-44 (Xiaomi), myron. Reproduce: `fv-unwrap <abl.img> pe.efi` then scan the PE.
